...
-spec new(iodata(), iodata(), iodata()) -> user().
...
-spec user_name(user()) -> iodata().
user_name(User) ->
  maps:get(user_name, User).

-spec user_name(user(), iodata()) -> user().
user_name(User, UserName) ->
  User#{user_name => UserName}.

-spec password(user()) -> iodata().
password(User) ->
  maps:get(password, User).

-spec password(user(), iodata()) -> user().
password(User, Password) ->
  User#{password => Password}.

-spec email(user()) -> iodata().
email(User) ->
  maps:get(email, User).

-spec email(user(), iodata()) -> user().
email(User, Email) ->
  User#{email => Email}.
...
-spec truncate_seconds(user()) -> user().
truncate_seconds(User) ->
  CreatedAt = cnf_utils:truncate_seconds(created_at(User)),
  UpdatedAt = cnf_utils:truncate_seconds(updated_at(User)),
  User#{updated_at => UpdatedAt, created_at => CreatedAt}.
