...
test_counts_votes_message(Config)  ->
  ...
  FunUp =
   fun(Id) ->
     cnf_vote_repo:upvote(Id, MsgId)
   end,
  ok = lists:foreach(FunUp, lists:seq(1, CountVotesUp)),
  FunDown =
   fun(Id) ->
     cnf_vote_repo:downvote(Id, MsgId)
    end,
  ok = lists:foreach(FunDown, lists:seq(1, CountVotesDown)),
